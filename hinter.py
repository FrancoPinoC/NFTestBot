...
class Hinter:
    ...
    async def add_hint(self, hintname, hint):
        if hintname in self.hints:
            await self.bot.say("A hint named '" + hintname + "' already exists.\n"
            ...
            await self.bot.say("Added hint '" + hintname + "'.")
    ...
    async def del_hint(self, hintname):
        if hintname not in self.hints:
            await self.bot.say("There is no hint named '" + hintname + "'.")
        else:
            del self.hints[hintname]
            await self.bot.say("Deleted hint '" + hintname + "'.")

    @commands.command()
    async def hint_pls(self, hintname):
        if hintname not in self.hints:
            await self.bot.say("Sorry, there's no hint for '" + hintname + "'.")
        else:
